...
//		}
		...
		public void ShowTab(MainTab	Tab) {
			...
			if (this.SandDockLayouts.Contains(Tab) &&
				!String.IsNullOrEmpty((string)this.SandDockLayouts[Tab]) ) {
				try {
...
//				_MainForm.Draw_Song_Preview_Image_Threaded();
		...
		private void SaveTabLayout(MainTab tab) {
			if (!this.SandDockLayouts.Contains(tab) ||
				String.IsNullOrEmpty((string)this.SandDockLayouts[tab])) { return; }
		...
		private string ReadTabLayout(MainTab tab) {
			...
			using (FileStream fs = File.Open(file, FileMode.OpenOrCreate, FileAccess.Read)) {
				using (StreamReader sr = new StreamReader(fs)) {
					return ConvertLayout2LocalCulture(sr.ReadToEnd());
				}
			}
		}

		/// <summary>
		/// Converts the SandDockManager XML layout string to a usable form.
		/// 
		/// SandDockManager.GetLayout() generates an XML string that uses the local culture ListSeparator
		/// when serializing some properties such as a 2D size. For example, if the ListSeparator is ","
		/// the XML contains: Size="100, 200"
		/// 
		/// If SandDockManager.SetLayout() is given an XML string that was generated in a Region which
		/// used a different ListSeparator than the current Region, DreamBeam will crash when SandDockManager
		/// tries to make use of that XML data (the SetLayout() call accepts any XML without complaints).
		/// 
		/// - XP and Vista only allow the user to select ";" or "," as the list separator by default.
		/// - The XML generated by SandDockManager doesn't use ";" or "," except as list separators (i.e.
		///   it doesn't use "," as a decimal separator)
		/// - The list separator is defined in the ControlPanel->RegionalOptions->Numbers->ListSeparator
		/// 
		/// We have no way of determining the ListSeparator on the system that generated the XML. We can
		/// only assume that it was ";" or "," (the user can set it to any crazy charater they want).
		/// 
		/// We'll do a brute-force search-n-replace of ";" and "," with whatever the current ListSeparator is.
		/// </summary>
		/// <param name="xmlStr"></param>
		/// <returns></returns>
		private string ConvertLayout2LocalCulture(string xmlStr) {
			if (String.IsNullOrEmpty(xmlStr)) return xmlStr;

			string separator = CultureInfo.CurrentCulture.TextInfo.ListSeparator;

			xmlStr = xmlStr.Replace(";", separator);
			xmlStr = xmlStr.Replace(",", separator);
			return xmlStr;
		}
